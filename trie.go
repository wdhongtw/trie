...
// TrieGen implements sparse radix (Patricia) trie.
// Makes zero allocation on Get and SearchPrefixIn operations and two allocations per Put
//
// Create it just as &TrieGen{} and add required data.
...
func (t *TrieGen[T]) PutString(prefix string, value T) {
...
func (t *TrieGen[T]) Put(newPrefix []byte, val T) (oldValue T) {
	...
	if ind == len(curPrefix) {
		...
		t.Children = &[256]*TrieGen[T]{} // it would have a child anyway
...
func (t *TrieGen[T]) getChildOrCreate(ind byte) *TrieGen[T] {
	if t.Children == nil {
		t.Children = &[256]*TrieGen[T]{}
		t.Children[ind] = &TrieGen[T]{}
...
func (t *TrieGen[T]) GetByString(key string) (T, bool) {
	return t.Get([]byte(key))
}

// Get searches for exactly matching key in trie
func (t *TrieGen[T]) Get(key []byte) (T, bool) {
	var out T
	ind := 0
...
func (t *TrieGen[T]) TakePrefix(str string) (prefix string, ok bool) {
...
func (t *TrieGen[T]) SearchPrefixInString(str string) (value T, prefixLen int, ok bool) {
...
func (t *TrieGen[T]) SearchPrefixIn(input []byte) (value T, prefixLen int, ok bool) {
	ind := 0
...
func (t *TrieGen[T]) Iterate(callback func(prefix []byte, value T)) {
	t.iterate(make([]byte, 0, 1024), callback)
}

func (t *TrieGen[T]) iterate(prefix []byte, callback func([]byte, T)) {
...
func (t *TrieGen[T]) SubTrie(mask []byte, keepPrefix bool) (subTrie *TrieGen[T], ok bool) {
	return t.subTrie(mask, keepPrefix, mask, 0)
}

func (t *TrieGen[T]) subTrie(mask []byte, keepPrefix bool, originalMask []byte, originalMaskInd int) (subTrie *TrieGen[T], ok bool) {
	var ind = 0
...
func (t *TrieGen[T]) GetAllByString(str string) []T {
...
//	-> [v0, v1, v2]
func (t *TrieGen[T]) GetAll(mask []byte) []T {
	var ind = 0
	for ind < len(mask) && ind < len(t.Prefix) && mask[ind] == t.Prefix[ind] {
		ind++
	}
